...
# prompt utilities (langchain is installed in venv)
from langchain.prompts import PromptTemplate
from langchain.output_parsers import StructuredOutputParser, ResponseSchema

# we avoid langchain to keep dependencies minimal; prompts are built manually


# set your API key in the environment or here if needed
...
def generate_time_frame_prompt(sentence_text, attribute_text):
    # build a simple string prompt without langchain
    general_instruction = (
        "Please do not extract anything outside of the given Sentence. "
        "The extracted phrase spans should directly be from the given Sentence text.\n\n"
    )
    sentence = "[Sentence]: " + sentence_text + "\n"
    time_frame = (
        "What is the specific time frame associated with "
        + attribute_text
        + " in the above sentence? Please be precise (e.g., within 14 days, past 6 months, etc.) "
          "If time information is not available, return 'NA'"
    )
    return general_instruction + sentence + time_frame
...
def generate_individual_diseases_prompt(sentence_text):
    ...
    sentence = "[Sentence]: " + sentence_text + "\n"
    individual_diseases = (
        "What are all the diseases mentioned in this sentence? List each disease in a new line."
    )
    return general_instruction + sentence + individual_diseases
...
def generate_individual_treatments_prompt(sentence_text):
    ...
    individual_treatments = (
        "What are all the treatment names, therapy names, medication or drug names, "
        "and procedure names mentioned in this sentence? List each of them in a new line."
    )
    return general_instruction + sentence + individual_treatments
...
def generate_inclusion_criteria_prompt(criteria_text_inclusion):
    ...
    # concatenate all pieces into a single string
    parts = [
        general_instruction_prompt.template,
        inclusion_criteria_text_prompt.template,
        criteria_of_interest_prompt.template,
        output_format_prompt.template,
        criteria_level_instructions_prompt.template,
        entity_type_classification_prompt.template,
        entity_type_attribute_relations_prompt.template,
    ]
    in_criteria_prompt = "\n".join(parts)
...
def generate_exclusion_criteria_prompt(criteria_text_exclusion):
    ...
    parts = [
        general_instruction_prompt.template,
        exclusion_criteria_text_prompt.template,
        criteria_of_interest_prompt.template,
        output_format_prompt.template,
        criteria_level_instructions_prompt.template,
        entity_type_classification_prompt.template,
...
def generate_response(prompt, n_retry, delay, max_tokens, tr_id, cri_type):
    # prompt is a dict with 'prompt' string and optionally 'output_parser'
    if cri_type == 'in':
        cri_type = 'Inclusion'
    else:
        cri_type = 'Exclusion'
    prompt_text = prompt['prompt'].replace("{disease}", " non-alcoholic steatohepatitis (NASH)")
...
def generate_response_for_partial_doc(prompt, output_p, n_retry, delay, max_tokens, tr_id, cri_type):
    if cri_type == 'in':
        ...
        cri_type = 'Exclusion'
    prompt_text = prompt.replace("{disease}", " non-alcoholic steatohepatitis (NASH)")
    for i in range(n_retry):
        try:
            resp = openai.ChatCompletion.create(
                model="gpt-4",
...
def process_half_text_response(half_text_response, df_half_output, cri_type, ph_str, link_str, tr_id, num_retry, delay_t, m_toks, criteria_text_sent):
    ...
    if half_text_response is None:
        # nothing was generated, keep existing dataframe unchanged
        return df_half_output
    # otherwise process the returned list of extracted criteria
...
if __name__ == '__main__':
    ...
    for filename in tqdm(files):
        # some XMLs may contain non‑cp949 characters; read as utf-8
        with open(filename, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            # some XMLs use <nctId> instead of <nct_id>
            nct_tag = trial_data.find('nct_id') or trial_data.find('nctId')
            if nct_tag is None:
                logging.warning(f"Could not locate trial id in {filename}")
                continue
            trial_id = nct_tag.text
            ...
            # first attempt legacy schema: <eligibility><criteria><textblock>
            all_eligibility = trial_data.find_all('eligibility')
            for eligibility in all_eligibility:
                criteria = eligibility.find('criteria')
                if criteria and criteria.find('textblock'):
                    criteria_text_block = criteria.find('textblock').text
                else:
                    criteria_text_block = None

                if criteria_text_block and 'inclusion criteria' in criteria_text_block.lower() and 'exclusion criteria' in criteria_text_block.lower():
            ...
            # second attempt newer schema: <eligibilityModule><eligibilityCriteria>
            if in_criteria_text is None and ex_criteria_text is None:
                elig_mod = trial_data.find('eligibilityModule')
                if elig_mod:
                    crit_tag = elig_mod.find('eligibilityCriteria')
                    if crit_tag:
                        criteria_text_block = crit_tag.get_text(separator="\n").strip()
                        if 'inclusion criteria' in criteria_text_block.lower() and 'exclusion criteria' in criteria_text_block.lower():
                            ...
                            # if no clear split, treat entire block as inclusion for now
                            in_criteria_text = criteria_text_block

            if in_criteria_text is None and ex_criteria_text is None:
                logging.warning('Trial ID: {} -- Either inclusion or exclusion criteria text is null.'.format(trial_id))
                continue

            phase_tag = trial_data.find('phase')
            phase_str = phase_tag.text if phase_tag is not None else 'NA'

            try:
                if '/' in phase_str:
                    f_p = phase_str.split('/')[0]
                    s_p = phase_str.split('/')[1]
                    if 'phase' in f_p.lower():
                        f_p = f_p.lower().split('phase')[1].strip()
                    if 'phase' in s_p.lower():
                        s_p = s_p.lower().split('phase')[1].strip()
                    phase_str = f_p + '/' + s_p
                else:
                    if 'phase' in phase_str.lower():
                        phase_str = phase_str.lower().split('phase')[1].strip()
            except Exception:
                # leave phase_str as-is if we can't parse it
                pass
            ...
            # choose append mode only if the file already exists
            excel_mode = "a" if os.path.exists(output_file_path) else "w"
            if excel_mode == "a":
                with pd.ExcelWriter(output_file_path, mode=excel_mode, engine="openpyxl", if_sheet_exists="overlay") as writer:
                    df_write.to_excel(writer, sheet_name=trial_id)
            else:
                with pd.ExcelWriter(output_file_path, mode=excel_mode, engine="openpyxl") as writer:
                    df_write.to_excel(writer, sheet_name=trial_id)
